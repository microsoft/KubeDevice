...
func printContainerAllocation(contName string, cont *types.ContainerInfo) {
	//utils.Logf(5, "Allocated", cont.Resources.Allocated)
...
func TestGrpAllocate1(t *testing.T) {
	...
	pod, podEx = createPod("pod2", 0.3,
		[]cont{
			{name: "Init0",
				res:            map[string]int64{string(gpuplugintypes.ResourceGPU): 1},
				expectedGrpLoc: map[string]string{"gpu/0": "gpu/dev4"}}},
		[]cont{
			{name: "Run0",
				res: map[string]int64{string(gpuplugintypes.ResourceGPU): 2},
				expectedGrpLoc: map[string]string{
					"gpu/0": "gpu/dev4",
					"gpu/1": "gpu/dev3"},
			},
